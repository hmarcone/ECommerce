...
namespace Web_ECommerce.Controllers
{
    ...
    public class UsuariosController : BaseController
    {
        ...
        public async Task<IActionResult> Edit(string id)
        {
            ...
            {
                new SelectListItem { Text = Enum.GetName(typeof(TipoUsuario), TipoUsuario.Comum), Value = Convert.ToInt32(TipoUsuario.Comum).ToString() },
                new SelectListItem { Text = Enum.GetName(typeof(TipoUsuario), TipoUsuario.Administrador), Value = Convert.ToInt32(TipoUsuario.Administrador).ToString() }
            };
